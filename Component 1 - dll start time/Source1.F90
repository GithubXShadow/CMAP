...
!Tripstarttime
real,allocatable:: ABMTripStartTime(:),ABMTempTime(:)
character(3)::Text
        ...
        if (validfirsttrip.eq.0) then 
            ...
            if (EndLoop.eq.1)  then
                ...
                do while(EndLoop1.eq.0.and.ll.lt.jjj)
                    ...
                    enddo
                enddo
                if(EndLoop1.eq.0) write(3,*) Traveller_ID(i),'No valid trip chains'   
            else
...
!                        write(3,*) "The trip start time is not assigned properly"
!                        pause
!                    endif
!                endif
!            endif
...
!enddo
!-------------------------------------------
!if the start time and the trip start time interval is not matching, change the start time interval
do i=1,index
    do j=1,TravellerNumTrips(i)
        ST=TravellerTripStartTimeInterval(i,j)
        if(ST.eq.0) then 
            write(3,*) "Traveller", i, j,"start time intervale erro"
        endif
        if(TripStartTime(i,j).gt.VectorStartTimeofTimeInterval(ST)+VectorLengthofTimeInterval(ST).or.TripStartTime(i,j).lt.VectorStartTimeofTimeInterval(ST)) then 
            if (TripStartTime(i,j).lt.VectorStartTimeofTimeInterval(2)) then 
                TravellerTripStartTimeInterval(i,j)=1
            else if (TripStartTime(i,j).gt.VectorStartTimeofTimeInterval(MaxNumTimeInterval)) then 
                TravellerTripStartTimeInterval(i,j)=MaxNumTimeInterval
            else 
                TravellerTripStartTimeInterval(i,j)=ceiling((TripStartTime(i,j)-120)/30.0)+1
            endif
        endif
        if(TravellerTripStartTimeInterval(i,j).gt.40.or.TravellerTripStartTimeInterval(i,j).lt.0) then 
            write(*,*)
        endif
    enddo
enddo
...
!Calculate the time only for joint trips
...
do i=1,index
    ...
    do j=1,TravellerNumTrips(i)  
        if(TravellerTripJointFlag(i,j).gt.0) then 
            !--------------------------------
            ...
            if(j.lt.TravellerNumTrips(i)) then
                ...
                if(TravellerActivityTime(i,j).lt.0) then 
                    ...
                    ABMTempTime(AbmTripIndex(i,j))=TripStartTime(i,j+1)-TripStartTime(i,j)
                endif
            else
                ...
                TravellerActivityTime(i,j)=(nint(ActivityTime*temp1))/temp1
                 if(TravellerActivityTime(i,j).lt.0) then 
                    TravellerActivityTime(i,j)=0.0
                    ABMTempTime(AbmTripIndex(i,j))=1440.0-TripStartTime(i,j)
                endif
            endif
        endif
    enddo
