...
!Variables and Parameters Clarifications
    ...
    character(30):: tour_category,type1,charcheck
    ...
    real:: LWTmp,ActivityTime,ActivityTimeTemp,temp1,temp3,temp,trip_travel_time
...
!Tripstarttime
real,allocatable:: ABMTripStartTime(:),ABMTempTime(:),ABMTripActivityTime(:)
        
character(3)::Text
character(50)::TextFinal
!-------------------------------------------------  
...
! To sort the trip back to the origin abm file
    allocate(ABMTripIndex(MaxNumTraveller,MaxNumTrips))
        ABMTripIndex(:,:)=0
!   allocate(ABMTravelTimeTemp(NumIndivTrips+NumJointTrips))
!    ABMTravelTimeTemp(:)=0
!Parking taz
    ...
    if (error.ne.-1) then      
        do while (error.ne.-1)   
            ...
            if (error.ne.-1) then
                ...
                zz=zz+1
                charcheck=trim(type1)
                if(trim(type1).eq.'Full-time-worker') then 
                    TravellerType(person_id)=1  
                elseif(trim(type1).eq.'Part-time-worker') then
                    TravellerType(person_id)=2
                elseif(trim(type1).eq.'University-student') then
                    TravellerType(person_id)=3
                elseif(trim(type1).eq.'Non-worker') then
                    TravellerType(person_id)=4
                elseif(trim(type1).eq.'Retired') then
                    TravellerType(person_id)=5
                elseif(trim(type1).eq.'Student-of-driving-age') then
                    TravellerType(person_id)=6
                elseif(trim(type1).eq.'Student-of-non-driving-age') then
                    TravellerType(person_id)=7
                elseif(trim(type1).eq.'Child-too-young-for-school') then
                    TravellerType(person_id)=8
                endif
                if(TravellerType(person_id).eq.0) then 
                    write(3,*) "Can not find correct person type for traveller", person_id 
                    pause
                endif
            endif
        enddo
...
enddo
trip_start_time=0
trip_activity_time=0

!-------------------------------------------------------------------       	
!Reading the individual trips from the input file, for each trip it will generate a traveller and add the first trip to it or if it has already been generated it will be added to the trips of that traveller   
    read(1,*)       
    index=0
    abm_trip_index=0
    error=1
    ...
    do while (error.ne.-1)   
        ...
        if (error.ne.-1) then
        read(Line,*,iostat=error) hh_id, person_id, person_num, tour_id, stop_id, inbound, tour_purpose, orig_purpose, dest_purpose, orig_maz, orig_walk_segment,orig_park_maz,dest_maz, dest_walk_segment, parking_taz, stop_period, trip_mode, tour_mode, tour_category, board_tap, alight_tap, orig_taz, dest_taz,orig_park_taz,dest_park_taz,trip_driver_passenger !,trip_start_time,trip_activity_time
        
        abm_trip_index=abm_trip_index+1
        !For Checking
!        if(trip_mode.eq.1) then 
!            zz=1
!        else 
!            zz=trip_driver_passenger
!        endif
!        if(zz.gt.0) write(22,*)person_id,orig_maz,dest_maz,stop_period,zz
        if ((trip_mode.eq.1).or.(trip_mode.eq.2)) trip_driver_passenger=person_num
        if ((trip_mode.ge.7).and.(trip_mode.le.10).or.(trip_mode.ge.13).and.(trip_mode.le.14)) trip_driver_passenger=-1
        !Read the driver information
            if (Traveller_index(person_id).eq.0) then !check if that traveller is already generated or not, if not genrate it and assign the first trip to it
                !Joint (mapping the person_id to hh_id and person number to be used in the Joint trip)
                PersonIDOfHH_PN(hh_id,person_num)=person_id                  
                index=index+1
                if (index.gt.MaxNumTraveller) then
                    write(3,*) 'Maximum  Number of Traveller is wrong, it is ', MaxNumTraveller, ' while we have at least ',index
                    pause
                endif
                ABMTripIndex(index,1)=abm_trip_index
                Traveller_index(person_id)=index
                Traveller_ID(index)=person_id
                TravellerNumTrips(index)=1
                TravellerTripMode(index,1)=trip_mode
                TravellerTripOriginMAZ(index,1)=orig_maz
                TravellerTripOriginTAZ(index,1)=orig_taz
                TravellerTripDestinationMAZ(index,1)=dest_maz
                TravellerTripDestinationTAZ(index,1)=dest_taz
                TravellerTripOriginPurpose(index,1)=orig_purpose
                TravellerTripDestinationPurpose(index,1)=dest_purpose
                TravellerTripStartTimeInterval(index,1)=stop_period
                TripDriverPassengerFlag(index,1)=trip_driver_passenger
                TravellerTripDestinationParkingTaz(index,1)=parking_taz
                TripStartTime(index,1)=trip_start_time
                TravellerActivityTime(index,1)=trip_activity_time
                !Joint
                !if ((TravellerTripMode(index,1).eq.1).or.(TravellerTripMode(index,1).eq.2)) TravellerTripDriver(index,1)=person_id   
                if (TripDriverPassengerFlag(index,1).gt.0) then 
                    TravellerTripDriver(index,1)=person_id 
                else         
                    TravellerTripDriver(index,1)=0
                endif
                !if ((TravellerTripMode(index,1).gt.0).and.(TravellerTripMode(index,1).lt.7)) CarNumTrip(i)=CarNumTrip(i)+1
                !if (TripDriverPassengerFlag(index,1).gt.0) CarNumTrip(i)=CarNumTrip(i)+1
                Traveller_HH_ID(index)=hh_id
                TravellerTripTourID(index,1)=tour_id+1
            else !if the traveller is generated before, add the trip for that traveller among the all other existing trips for that traveller (sorted based on the start time)
                TravellerNumTrips(Traveller_index(person_id))=TravellerNumTrips(Traveller_index(person_id))+1
                if (TravellerNumTrips(Traveller_index(person_id)).gt.RealMaxNumTrips) RealMaxNumTrips=TravellerNumTrips(Traveller_index(person_id))
                if (TravellerNumTrips(Traveller_index(person_id)).gt.MaxNumTrips) then
                    write(3,*) 'Maximum Number of Trips is wrong, it is ', MaxNumTrips, ' while we have at least ', TravellerNumTrips(Traveller_index(person_id))
                    pause
                endif
                j=1
                EndofLoop=0
                !Search for the proper place for the trip based on the start time
                do while(j.lt.TravellerNumTrips(Traveller_index(person_id)).and.EndofLoop.eq.0)  !search over all the existing trips unless the proper place is found
                    if (TravellerTripStartTimeInterval(Traveller_index(person_id),j).gt.stop_period) then !find the proper place for the current trip based on the start time
                        do jj=TravellerNumTrips(Traveller_index(person_id))-1,j,-1 !shift all the remaining trips one trip ahead
                            TravellerTripMode(Traveller_index(person_id),jj+1)=TravellerTripMode(Traveller_index(person_id),jj)
                            TravellerTripOriginMAZ(Traveller_index(person_id),jj+1)=TravellerTripOriginMAZ(Traveller_index(person_id),jj)
                            TravellerTripOriginTAZ(Traveller_index(person_id),jj+1)=TravellerTripOriginTAZ(Traveller_index(person_id),jj)
                            TravellerTripDestinationMAZ(Traveller_index(person_id),jj+1)=TravellerTripDestinationMAZ(Traveller_index(person_id),jj)
                            TravellerTripDestinationTAZ(Traveller_index(person_id),jj+1)=TravellerTripDestinationTAZ(Traveller_index(person_id),jj)
                            TravellerTripOriginPurpose(Traveller_index(person_id),jj+1)=TravellerTripOriginPurpose(Traveller_index(person_id),jj)
                            TravellerTripDestinationPurpose(Traveller_index(person_id),jj+1)=TravellerTripDestinationPurpose(Traveller_index(person_id),jj)
                            TravellerTripStartTimeInterval(Traveller_index(person_id),jj+1)=TravellerTripStartTimeInterval(Traveller_index(person_id),jj)
                            TripStartTime(Traveller_index(person_id),jj+1)=TripStartTime(Traveller_index(person_id),jj)
                            TravellerActivityTime(Traveller_index(person_id),jj+1)=TravellerActivityTime(Traveller_index(person_id),jj)
                            TripDriverPassengerFlag(Traveller_index(person_id),jj+1)=TripDriverPassengerFlag(Traveller_index(person_id),jj)
                            TravellerTripDestinationParkingTaz(Traveller_index(person_id),jj+1)=TravellerTripDestinationParkingTaz(Traveller_index(person_id),jj)
                            
                            !Simulation to abm
                            ABMTripIndex(Traveller_index(person_id),jj+1)=ABMTripIndex(Traveller_index(person_id),jj)
                            !Joint
                            TravellerTripDriver(Traveller_index(person_id),jj+1)=TravellerTripDriver(Traveller_index(person_id),jj)
                            TravellerTripTourID(Traveller_index(person_id),jj+1)=TravellerTripTourID(Traveller_index(person_id),jj)
                            TravellerTripJointFlag(Traveller_index(person_id),jj+1)=TravellerTripJointFlag(Traveller_index(person_id),jj)          
                        enddo
                        !assign the current trip to the found place 
                        TravellerTripMode(Traveller_index(person_id),j)=trip_mode
                        TravellerTripOriginMAZ(Traveller_index(person_id),j)=orig_maz
                        TravellerTripOriginTAZ(Traveller_index(person_id),j)=orig_taz
                        TravellerTripDestinationMAZ(Traveller_index(person_id),j)=dest_maz
                        TravellerTripDestinationTAZ(Traveller_index(person_id),j)=dest_taz
                        TravellerTripOriginPurpose(Traveller_index(person_id),j)=orig_purpose
                        TravellerTripDestinationPurpose(Traveller_index(person_id),j)=dest_purpose
                        TravellerTripStartTimeInterval(Traveller_index(person_id),j)=stop_period
                        TripDriverPassengerFlag(Traveller_index(person_id),j)=trip_driver_passenger
                        ABMTripIndex(Traveller_index(person_id),j)=abm_trip_index
                        TravellerTripDestinationParkingTaz(Traveller_index(person_id),j)=parking_taz
                        TripStartTime(Traveller_index(person_id),j)=trip_start_time
                        TravellerActivityTime(Traveller_index(person_id),j)=trip_activity_time
                        !Joint
!                        if ((TravellerTripMode(Traveller_index(person_id),j).eq.1).or.(TravellerTripMode(Traveller_index(person_id),j).eq.2)) then
!                            TravellerTripDriver(Traveller_index(person_id),j)=person_id           
!                        else   
!                            TravellerTripDriver(Traveller_index(person_id),j)=0
!                        endif
                        if(TripDriverPassengerFlag(Traveller_index(person_id),j).gt.0) then
                            TravellerTripDriver(Traveller_index(person_id),j)=person_id
                        else
                            TravellerTripDriver(Traveller_index(person_id),j)=0
                        endif
                        !if ((TravellerTripMode(Traveller_index(person_id),j).gt.0).and.(TravellerTripMode(Traveller_index(person_id),j).lt.7)) CarNumTrip(i)=CarNumTrip(i)+1
                        !if (TripDriverPassengerFlag(Traveller_index(person_id),j).gt.0) CarNumTrip(i)=CarNumTrip(i)+1  !For checking only
                        TravellerTripTourID(Traveller_index(person_id),j)=tour_id+1
                        EndofLoop=1
                    elseif (j.eq.TravellerNumTrips(Traveller_index(person_id))-1) then !if the proper place for current trip is not found yet, set it as the last trip
                        TravellerTripMode(Traveller_index(person_id),j+1)=trip_mode
                        TravellerTripOriginMAZ(Traveller_index(person_id),j+1)=orig_maz
                        TravellerTripOriginTAZ(Traveller_index(person_id),j+1)=orig_taz
                        TravellerTripDestinationMAZ(Traveller_index(person_id),j+1)=dest_maz
                        TravellerTripDestinationTAZ(Traveller_index(person_id),j+1)=dest_taz
                        TravellerTripOriginPurpose(Traveller_index(person_id),j+1)=orig_purpose
                        TravellerTripDestinationPurpose(Traveller_index(person_id),j+1)=dest_purpose
                        TravellerTripStartTimeInterval(Traveller_index(person_id),j+1)=stop_period
                        TripDriverPassengerFlag(Traveller_index(person_id),j+1)=trip_driver_passenger
                        ABMTripIndex(Traveller_index(person_id),j+1)=abm_trip_index
                        TravellerTripDestinationParkingTaz(Traveller_index(person_id),j+1)=parking_taz
                        TripStartTime(Traveller_index(person_id),j+1)=trip_start_time
                        TravellerActivityTime(Traveller_index(person_id),j+1)=trip_activity_time
                        !Joint
                        !if ((TravellerTripMode(Traveller_index(person_id),j+1).eq.1).or.(TravellerTripMode(Traveller_index(person_id),j+1).eq.2)) TravellerTripDriver(Traveller_index(person_id),j+1)=person_id             
                        if(TripDriverPassengerFlag(Traveller_index(person_id),j+1).gt.0) then
                            TravellerTripDriver(Traveller_index(person_id),j+1)=person_id
                        else
                            TravellerTripDriver(Traveller_index(person_id),j+1)=0
                        endif
                        !if ((TravellerTripMode(Traveller_index(person_id),j+1).gt.0).and.(TravellerTripMode(Traveller_index(person_id),j+1).lt.7)) CarNumTrip(i)=CarNumTrip(i)+1
                        !if (TripDriverPassengerFlag(Traveller_index(person_id),j+1).gt.0) CarNumTrip(i)=CarNumTrip(i)+1 for checking only
                        TravellerTripTourID(Traveller_index(person_id),j+1)=tour_id+1
                    endif
                    j=j+1                  
                enddo
            endif
...
!-------------------------------------------------------------------       	    
!Joint
!Reading the joint tour file from ABM output to make a map from "household ID" and "tour ID" to the "person number" in the family for each tour participant 
!Note: in this file there is a combination of comma and space as the separator. The tour participants are separted with the space and we do not know the number of tour participant for each tour. Others are separated by comma.
    read(11,*)
    error=1
    do while (error.ne.-1)
        read(11,'(a)',iostat=error) JointTourLine
        if (error.ne.-1) then
    ...
    close(11)
!-------------------------------------------------------------------       	    
!Reading the joint trips information from ABM output file. Each joint trip will be added to the list of trips for all the tour participants.
    read(12,*)       
    error=1
    ...
    do while (error.ne.-1)   
        ...
        if (error.ne.-1) then
            ...
            !keep updating the abm tirp index
!            if(trip_mode.eq.1) then 
!                zz=1
!            else 
!                zz=trip_driver_passenger
...
!            if(zz.gt.0) write(22,*)person_id,orig_maz,dest_maz,stop_period,zz
            abm_trip_index=abm_trip_index+1
            if (NumTourParticipant(hh_id,tour_id+1).ne.num_participants) then
                write(3,*) 'Number of tour participants is not consistent in joint trip relative to the joint tour file for the hosehole_id and tour_id ', hh_id, tour_id
                pause
            endif            
            !find the driver if the mode trip is a shared ride, as the person with the largest number of individual trips with car (single or shared)
            if (trip_driver_passenger.ne.-1) then 
                do k=1,NumTourParticipant(hh_id,tour_id+1) !search over all the tour participants and assign the traveller with the maximum number of individual car trips (shared or single) as the driver
                    person_id=PersonIDOfHH_PN(hh_id,PersonNumOfHH_TourID(hh_id,tour_id+1,k))
                    if (person_id.ne.0) then
                        if (person_id.eq.PersonIDOfHH_PN(hh_id,trip_driver_passenger)) then 
                            !TripDriverPassengerFlag(index,1)=trip_driver_passenger
                            driver_id=person_id
                        endif
                    else
                        NonExistingTraveller= NonExistingTraveller + 1     
                    endif
                enddo
                if (driver_id.eq.0) then
                    NonDriver=1
                endif
            endif
!            if(parking_taz.gt.0) then 
!                if(dest_maz.ne.parking_taz) write(3,*) driver_id,'inconsistent type 4'
!            endif
            !Adding the current joint trip to the list of trips of each tour participants                         
            do k=1,NumTourParticipant(hh_id,tour_id+1)                
                person_id=PersonIDOfHH_PN(hh_id,PersonNumOfHH_TourID(hh_id,tour_id+1,k))
                if (person_id.ne.0) then !if it is a nonexisting traveller we will just ignore that person                                  
                    if (Traveller_index(person_id).eq.0) then !for the case that we can read the person_id of a person number of a household for a nonexisting person in the individual trips list from another input file. In this case we add one travellers to the whole simulation and just assign the joint trip or trips for that traveller
                        index=index+1
                        if (index.gt.MaxNumTraveller) then
                            write(3,*) 'Maximum  Number of Traveller is wrong, it is ', MaxNumTraveller, ' while we have at least ',index 
                            pause
                        ...
                        TravellerTripDestinationPurpose(index,1)=dest_purpose
                        TravellerTripStartTimeInterval(index,1)=stop_period             
                        TravellerTripDriver(index,1)=driver_id 
                        Traveller_HH_ID(index)=hh_id
                        TravellerTripTourID(index,1)=tour_id+1 
                        TravellerTripJointFlag(index,1)=1
                        ABMTripIndex(index,1)=abm_trip_index
                        TravellerTripDestinationParkingTaz(index,1)=parking_taz
                        TripStartTime(index,1)=trip_start_time
                        TravellerActivityTime(index,1)=trip_activity_time
                        if(trip_mode.gt.6) then 
                            TripDriverPassengerFlag(index,1)=-1
                        elseif(trip_mode.gt.0.and.trip_mode.lt.7) then
                            if (person_id.eq.driver_id) then 
                                TripDriverPassengerFlag(index,1)=trip_driver_passenger
                            else
                                TripDriverPassengerFlag(index,1)=-1    
                            endif
                    ...
                    else ! the traveller already exist, so we just add the joint trip for that traveller in the pr0per place of its trip list based on the start time
                        TravellerNumTrips(Traveller_index(person_id))=TravellerNumTrips(Traveller_index(person_id))+1
                        if (TravellerNumTrips(Traveller_index(person_id)).gt.RealMaxNumTrips) RealMaxNumTrips=TravellerNumTrips(Traveller_index(person_id))
                        if (TravellerNumTrips(Traveller_index(person_id)).gt.MaxNumTrips) then
                            write(3,*) 'Maximum Number of Trips is wrong, it is ', MaxNumTrips, ' while we have at least ', TravellerNumTrips(Traveller_index(person_id))
                        ...
                        do while(j.lt.TravellerNumTrips(Traveller_index(person_id)).and.EndofLoop.eq.0) !search over all the existing trips unless the proper place is found
                            if (TravellerTripStartTimeInterval(Traveller_index(person_id),j).gt.stop_period) then  !find the proper place for the current trip based on the start time
                                ...
                                TravellerTripDestinationPurpose(Traveller_index(person_id),j)=dest_purpose
                                TravellerTripStartTimeInterval(Traveller_index(person_id),j)=stop_period
                                TravellerTripDriver(Traveller_index(person_id),j)=driver_id
                                TravellerTripTourID(Traveller_index(person_id),j)=tour_id+1
                                TravellerTripJointFlag(Traveller_index(person_id),j)=1
                                ABMTripIndex(Traveller_index(person_id),j)=abm_trip_index
                                TravellerTripDestinationParkingTaz(Traveller_index(person_id),j)=parking_taz
                                TripStartTime(Traveller_index(person_id),j)=trip_start_time
                                TravellerActivityTime(Traveller_index(person_id),j)=trip_activity_time
                                if(trip_mode.gt.6) then 
                                    TripDriverPassengerFlag(Traveller_index(person_id),j)=-1
                                elseif(trip_mode.gt.0.and.trip_mode.lt.7) then
                                    if (person_id.eq.driver_id) then 
                                        TripDriverPassengerFlag(Traveller_index(person_id),j)=trip_driver_passenger
                                    else
                            ...
                            elseif (j.eq.TravellerNumTrips(Traveller_index(person_id))-1) then !if the proper place for current trip is not found yet, set it as the last trip
                                ...
                                TravellerTripDestinationPurpose(Traveller_index(person_id),j+1)=dest_purpose
                                TravellerTripStartTimeInterval(Traveller_index(person_id),j+1)=stop_period                                                       
                                TravellerTripDriver(Traveller_index(person_id),j+1)=driver_id
                                TravellerTripTourID(Traveller_index(person_id),j+1)=tour_id+1
                                TravellerTripJointFlag(Traveller_index(person_id),j+1)=1    
                                ABMTripIndex(Traveller_index(person_id),j+1)=abm_trip_index
                                TravellerTripDestinationParkingTaz(Traveller_index(person_id),j+1)=parking_taz
                                TripStartTime(Traveller_index(person_id),j+1)=trip_start_time
                                TravellerActivityTime(Traveller_index(person_id),j+1)=trip_activity_time
                                if(trip_mode.gt.6) then 
                                    TripDriverPassengerFlag(Traveller_index(person_id),j+1)=-1
                                elseif(trip_mode.gt.0.and.trip_mode.lt.7) then
                                    if (person_id.eq.driver_id) then 
                                        TripDriverPassengerFlag(Traveller_index(person_id),j+1)=trip_driver_passenger
                                    else
            ...
            if (trip_driver_passenger.ne.-1) then 
                if (driver_id.eq.0) then 
                    NoNTravelerDriver=NoNTravelerDriver+1
                    !pause
                endif
            endif                        
        endif    
...
!After reading the individual trips and joint trips, they are all sorted by start time interval, now we need to resort the trips in the same time interval, based on the origin-destination, zone and purpose of trip consistency
   ...
   do i=1,index ! search over all the identified travellers from individual trip list of ABM output
        ...
        trip_start_time=TripStartTime(i,jj)
        trip_activity_time=TravellerActivityTime(i,jj)
        !Joint
        ...
        TripStartTime(i,jj)=TripStartTime(i,j)
        TravellerActivityTime(i,jj)=TravellerActivityTime(i,j)
        !Joint
        ...
        TravellerActivityTime(i,j)=trip_activity_time
        !Joint
...
enddo
!Reduce the length of the last time interval by 1, so that the trip start time won't be equal to 1440
VectorLengthofTimeInterval(MaxNumTimeInterval)=VectorLengthofTimeInterval(MaxNumTimeInterval)-1
close(5)
!-------------------------------------------------------------------
!Create the start time for each trip
!allocate(AbmTripStartTime(NumIndivTrips+NumJointTrips))
!    AbmTripStartTime(:)=0
!allocate(ABMTempTime(NumIndivTrips+NumJointTrips))
!    ABMTempTime(:)=0 
!do i=1,index
!    jj1=1
!    jj2=1
...
!           
!         
!            if(TravellerTripJointFlag(i,jj2).eq.1.and.jointcounter.eq.0.and.AbmTripStartTime(ABMTripIndex(i,jj2)).gt.0) then 
!                jointcounter=jj2
!            endif
!      
...
!        do jjj=jj1,jj2-1  
!            if(AbmTripStartTime(ABMTripIndex(i,jjj)).eq.0) then 
!                if(jointcounter.eq.0.or.jjj.eq.jointcounter) then 
!                    TripStartTime(i,jjj)=(VectorStartTimeofTimeInterval(ST)+1+(VectorLengthofTimeInterval(ST)-2)/TNInInterval*(jjj-jj1))+(VectorLengthofTimeInterval(ST)-2)/TNInInterval*rand(0)
!                elseif(jjj.lt.jointcounter) then
!                    TripStartTime(i,jjj)=(VectorStartTimeofTimeInterval(ST)+1+(AbmTripStartTime(ABMTripIndex(i,jointcounter))-VectorStartTimeofTimeInterval(ST)-2)/(jointcounter-jj1)*(jjj-jj1))+(AbmTripStartTime(ABMTripIndex(i,jointcounter))-VectorStartTimeofTimeInterval(ST)-2)/(jointcounter-jj1)*rand(0)
!                elseif(jjj.gt.jointcounter) then
!                    TripStartTime(i,jjj)=(AbmTripStartTime(ABMTripIndex(i,jointcounter))+1+(VectorLengthofTimeInterval(ST)-(AbmTripStartTime(ABMTripIndex(i,jointcounter))-VectorStartTimeofTimeInterval(ST))-2)/(jj2-jointcounter)*(jjj-jointcounter))+(VectorLengthofTimeInterval(ST)-(AbmTripStartTime(ABMTripIndex(i,jointcounter))-VectorStartTimeofTimeInterval(ST))-2)/(jj2-jointcounter)*rand(0)
!                endif
!                TripStartTime(i,jjj)=nint(TripStartTime(i,jjj))*10.0/10.0
!                if(TripStartTime(i,jjj).lt.0.1) TripStartTime(i,jjj)=1
!                if(jjj.ne.1) then 
!                    if(TripStartTime(i,jjj).lt.TripStartTime(i,jjj-1)) then 
!                        write(101,*) "The trip start time is not assigned properly",i,jjj,TripStartTime(i,jjj),TripStartTime(i,jjj-1)
!                        !pause
!                    endif
!                endif
!                AbmTripStartTime(ABMTripIndex(i,jjj))=TripStartTime(i,jjj)
!                
!            else
!                TripStartTime(i,jjj)=AbmTripStartTime(ABMTripIndex(i,jjj) )
!                if(jjj.ne.1) then 
!                    if(TripStartTime(i,jjj).lt.TripStartTime(i,jjj-1)) then 
!                        write(101,*) "The trip start time is not assigned properly",i,jjj,TripStartTime(i,jjj),TripStartTime(i,jjj-1)
!                        !pause
...
allocate(ABMTempTime(NumIndivTrips+NumJointTrips))
    ABMTempTime(:)=0 
 allocate(ABMTripActivityTime(NumIndivTrips+NumJointTrips))
    ABMTempTime(:)=0    
do i=1,index
    jj1=1
    jj2=1
    do while (jj2.le.TravellerNumTrips(i))
        ...
        do jjj=jj1,jj2-1        
            if(AbmTripStartTime(ABMTripIndex(i,jjj)).eq.0) then   
                ...
                TripStartTime(i,jjj)=nint(TripStartTime(i,jjj))*10.0/10.0
                if(jjj.gt.1) then 
                    if(TripStartTime(i,jjj).ne.1440.and.TripStartTime(i,jjj).eq.TripStartTime(i,jjj-1))then 
                        TripStartTime(i,jjj)=TripStartTime(i,jjj)+1
                    endif
                endif
                if(TripStartTime(i,jjj).lt.0.1) TripStartTime(i,jjj)=1
                AbmTripStartTime(ABMTripIndex(i,jjj))=TripStartTime(i,jjj)
                if(jjj.ne.1) then 
                    if(TripStartTime(i,jjj).le.TripStartTime(i,jjj-1)) then 
                        write(101,*) "The trip start time is not assigned properly",jjj,Traveller_ID(i)
                        ValidTraveller(i)=0
                    endif
                endif
            else
...
do D=1,NumSkimSuperZone
    write(Text,'(i3)') D
    TextFinal='vehicularPNRSkim//'// trim(adjustl(Text)) // '.dat'
    open(file=trim(TextFinal),unit=31)    
    read(31,*) 
    read(31,*)     
    if (NumIteration.gt.0) then
      do O=1,NumSkimOriginZone
        ...
        write(*,*) ODTATT_Array(O,D,T)%Psize,NumOfVOTGs
        if(ODTATT_Array(O,D,T)%Psize.ne.NumOfVOTGs.or.ODTATT_Array(O,D,T)%Psize.eq.0) then 
            write(*,*)ODTATT_Array(O,D,T)%Psize,NumOfVOTGs
        endif
        call ODTATTAllocate(O,D,T,NumOfVOTGs)
         do i=1,NumOfVOTGs
           call ODTATTInsert(O,D,T,i,TempVOT(i),TempCost(i),TempTime(i),TempDistance(i))
           write(*,*) ODTATT_Array(O,D,T)%P(1)%Time
         enddo  
...
write(*,*) "Read the Transit Skim Files"
...
do i=1,index
    ...
    do j=1,TravellerNumTrips(i)  !go over all the trips except the last trip
        !--------------------------------
        !calculate the estimate trip planned time
        if(i.eq.1815541.and.j.eq.4) then 
            write(*,*)
        endif
        temp=0
        if(TravellerTripMode(i,j).le.6.or.TravellerTripMode(i,j).ge.13) then  !if the trip is a car trip, calculate the travel time based on skim
            ...
            if(pathindex.eq.0) then !Read from the vehicularPNRSkim.dat
                ...
                do while(ValueofTime(Traveller_ID(i)).gt.ODTATT_Array(TravellerTripOriginTAZ(i,j),SuperZoneMap(TravellerTripDestinationTAZ(i,j),2),SpSkimTime)%P(vv)%VOT)
                    vv=vv+1
                enddo
                temp=ODTATT_Array(TravellerTripOriginTAZ(i,j),SuperZoneMap(TravellerTripDestinationTAZ(i,j),2),SpSkimTime)%P(vv)%Time
                
            else  !Read from the link travel time and turn penalty
                currentDYNASMARTtime=TripStartTime(i,j)
                do kk=1,NumIntraSuperzoneZone(pathindex)-2
                    !Add all link travel time together
                    nodea=PathBank(pathindex,kk)
                    nodeb=PathBank(pathindex,kk+1)
                    nodec=PathBank(pathindex,kk+2)
                    upstreamlinkNum=LinkNumber(NodeIndex(nodea),NodeIndex(nodeb))
                    currentDYNASMARTtimeinterval=ceiling(currentDYNASMARTtime/LinkTimeInterval)  !change the 10 to a variable
                    if(currentDYNASMARTtimeinterval.gt.NumLinkInterval) currentDYNASMARTtimeinterval=NumLinkInterval
                    !currentDYNASMARTtime=currentDYNASMARTtime+LinkTravelTime(upstreamlinkNum,currentDYNASMARTtimeinterval)
                ...
                currentDYNASMARTtimeinterval=ceiling(currentDYNASMARTtime/LinkTimeInterval)
                if(currentDYNASMARTtimeinterval.gt.NumLinkInterval) currentDYNASMARTtimeinterval=NumLinkInterval
                if(currentDYNASMARTtimeinterval.eq.0) write(*,*) i,j,kk,currentDYNASMARTtime,currentDYNASMARTtime/10,ceiling(currentDYNASMARTtime/10),currentDYNASMARTtimeinterval,1
                currentDYNASMARTtime=currentDYNASMARTtime+LinkTravelTime(LinkNumber(nodea,nodeb),currentDYNASMARTtimeinterval)
                temp=currentDYNASMARTtime-TripStartTime(i,j)
            endif
        elseif(TravellerTripMode(i,j).eq.7) then 
            temp=10+rand(0)*5
        elseif(TravellerTripMode(i,j).eq.8) then 
            temp=15+rand(0)*5
        elseif(TravellerTripMode(i,j).le.10) then 
            if(TransitMazTazFlag.eq.0) then 
                otranz=TravellerTripOriginMAZ(i,j)
                dtranz=TravellerTripDestinationMAZ(i,j)
            elseif(TransitMazTazFlag.eq.1) then
            ...
            m=ceiling(rand(0)*NumTransitWalkZoneCa(otranz))
            n=ceiling(rand(0)*NumTransitWalkZoneCa(dtranz))
            !Calculate the time of driving part
            otap=TransitWalkZone(otranz,m)
            dtap=TransitWalkZone(dtranz,n)
            if(otap.eq.0) then 
            ...
            !Travel Time of the first part of walking
            walklegtime=TransitSkimWalkTime(otranz,m)
            !Travel time of transit part
            PRNTransitStartTime=walklegtime+TripStartTime(i,j)
            if(otap.eq.0) then
            ...
            walklegtime1=TransitSkimWalkTime(dtranz,n)
            temp=walklegtime+transitlegtime+walklegtime1
        elseif(TravellerTripMode(i,j).le.12) then 
            temp=EsPNRTravelTime(i,j)
        endif
        if(temp.ne.0)then 
            temp=nint(temp)*10.0/10.0
        else
            temp=1.0
        endif
        if(ABMTempTime(AbmTripIndex(i,j)).eq.0) then 
           
            ABMTempTime(AbmTripIndex(i,j))=temp
            if(ABMTempTime(AbmTripIndex(i,j)).lt.0) write(3,*) "The planned travel time of traveller", Traveller_ID(i),"trip", j, "is negative"
!        else
!            if(j.lt.TravellerNumTrips(i)) then
!                ActivityTime=(TripStartTime(i,j+1)-TripStartTime(i,j))-ABMTempTime(AbmTripIndex(i,j))
!                if(ActivityTime.lt.0) then 
!                    write(101,*) "Trip Activity time is not assigned properly",Traveller_ID(i)
!                endif
!            else
...
!            endif
        ...
        if(j.lt.TravellerNumTrips(i)) then
            ...
            !if(ActivityTime.lt.0) then ActivityTime=0.0
            TravellerActivityTime(i,j)=ActivityTime
            if(TravellerActivityTime(i,j).lt.0) then 
                TravellerActivityTime(i,j)=1.0
                ABMTempTime(AbmTripIndex(i,j))=TripStartTime(i,j+1)-TripStartTime(i,j)-1
                ABMTripActivityTime(AbmTripIndex(i,j))=1
            endif
        else
            ActivityTime=1440.0-TripStartTime(i,j)-ABMTempTime(AbmTripIndex(i,j))
            TravellerActivityTime(i,j)=(nint(ActivityTime*temp1))/temp1
             if(TravellerActivityTime(i,j).lt.0) then 
                TravellerActivityTime(i,j)=1.0
                ABMTempTime(AbmTripIndex(i,j))=1440.0-TripStartTime(i,j)-1
                ABMTripActivityTime(AbmTripIndex(i,j))=1
            endif
        endif
    enddo
...
!            StartTime=(nint(TravellerStartTime(i)*temp1))/temp1
	        ...
	        write(8,12) counter,i,Traveller_ID(i),StartTime,TravellerNumTrips(i),TravellerTripOriginTAZ(i,1),ValidCar(i),FirstCarTrip(i,1),FirstTransitTrip(i),ValueOfTime(Traveller_ID(i)),TravellerType(Traveller_ID(i))
...
!                endif
                ...
                write(8,13) TravellerTripDestinationTAZ(i,j),ABMTripActivityTime(AbmTripIndex(i,j)),TravellerTripMode(i,j),TravellerTripOriginPurpose(i,j),TravellerTripDestinationPurpose(i,j),TravellerTripOriginMAZ(i,j),TravellerTripOriginTAZ(i,j),TravellerTripDestinationMAZ(i,j),TravellerTripDestinationTAZ(i,j),TripDriverPassengerFlag(i,j),TravellerTripJointFlag(i,j),TravellerTripParkRideFlag(i,j),TravellerTripStartTimeInterval(i,j),TripStartTime(i,j),ABMTripIndex(i,j),TravellerTripDriver(i,j),TravellerTripDestinationParkingTaz(i,j)
            enddo
            if(sum(TravellerActivityTime(i,:)).gt.1440) then 
                write(3,*) "Traveller", Traveller_ID(i),"'s total activity time is larger than 1440"
            endif
       enddo            
   enddo
   ...
   do tt=1,MaxNumTimeInterval
       ...
       do k=1,NumTransitPassengerInTimeInterval(tt)
            ...
            do j=1,TravellerNumTrips(i)   !go over all the trips   
                ...
                if (TravellerTripMode(i,j).ge.9.and.TravellerTripMode(i,j).le.12) then
                    TransitTripCounter=TransitTripCounter+1
...
!Now write the new transit setting
open(file='TransitSetting.dat',unit=30)
write(30,*)TransitMazTazFlag
write(30,*)drivingweight
write(30,*)walkingweight
write(30,*)TransitSkimTimeIntervalLength
write(30,*)MaxNumTransitSkimTimeInterval
write(30,*)WalkSpeed
write(30,*) int(TransitTripCounter)
close(30)
!----------------------------------------
...
!            endif
            ...
            realj=j
            temp1=rand(0)                    
            CumulativeActivityTime=TravellerActivityTime(i,j) !TripStartTime(i,j+1)-TripStartTime(i,j)-temp3*temp1
...
!        if(TripDriverPassengerFlag(i,j).gt.0.and.SuperZoneMap(TravellerTripDestinationTAZ(i,j),2).eq.SuperZoneMap(TravellerTripOriginTAZ(i,j),2)) alex=alex+1
!        if(SuperZoneMap(TravellerTripDestinationTAZ(i,j),2).eq.SuperZoneMap(TravellerTripOriginTAZ(i,j),2)) alex1=alex1+1
!        
!    enddo
!        alex2=alex2+TravellerNumTrips(i)
!    enddo
!    write(*,*) alex,alex1
...
12 format(3i10,f8.2,5i6,f8.4,i8)
