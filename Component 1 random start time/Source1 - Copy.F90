...
integer,allocatable::TravellerParkRideNum(:),MaztoTazMap(:),PNRMAZ(:,:),TransitZone(:,:),NumTransitZoneCa(:),TravellerTripOriginTransitMaz(:,:),TravellerTripDestinationTransitMaz(:,:)
!integer,allocatable::TravellerTripOriginTransitMAZ(:,:),TravellerTripDestinationTransitMAZ(:,:)
!Path bank
integer,allocatable::IntraSuperzonePathIndex(:,:),PathBank(:,:),NumIntraSuperzoneZone(:),UpStreamLink(:),DownStreamLink(:),zonetozonetraveltime(:,:,:)
!TDSP Skim
...
!Pathbank
    open(file='output_path.dat',unit=13)
...
do while (error.ne.-1) 
    ...
    read(26,*,iostat=error) maz1,maz2
    if(error.ne.-1) then
        if(maz1.eq.maztemp) then 
...
!ParkRide
    ...
    allocate(PNRMAZ(NumPairPNRMAZ,2))
        PNRMAZ(:,:)=0
    allocate(TransitZone(NumMaz,NumTransitOption))
        TransitZone(:,:)=0
    allocate(NumTransitZoneCa(NumMaz))
        NumTransitZoneCa(:)=0
...
do i=1,NumPairPNRMAZ
    read(26,*,iostat=error)PNRMAZ(i,1),PNRMAZ(i,2)
    if(error.ne.-1) then
        if(PNRMAZ(i,1).eq.maztemp) then 
            NumTransitOptionTemp=NumTransitOptionTemp+1
            NumTransitZoneCa(PNRMAZ(i,1))=NumTransitOptionTemp
        else
            NumTransitOptionTemp=1
            maztemp=PNRMAZ(i,1)
            NumTransitZoneCa(PNRMAZ(i,1))=NumTransitOptionTemp
    ...
    TransitZone(PNRMAZ(i,1),NumTransitOptionTemp)=PNRMAZ(i,2)
enddo
close(26)
open(file='PNRSkim_AB.dat',unit=26)
...
do i=1,index
    ...
    do while (j.le.TravellerNumTrips(i))
        if ((TravellerTripMode(i,j).eq.11).or.(TravellerTripMode(i,j).eq.12)) then
            ...
            if (TravellerTripParkRideFlag(i,j).eq.3) TravellerTripParkRideFlag(i,j)=1
            if (TravellerTripParkRideFlag(i,j).eq.4) TravellerTripParkRideFlag(i,j)=2
            if(TravellerTripParkRideFlag(i,j).eq.1) then
                randomtrans=ceiling(rand(0)*NumTransitZoneCa(TravellerTripOriginMAZ(i,j)))
                TravellerTripOriginTransitMAZ(i,j)=TransitZone(TravellerTripOriginMAZ(i,j),randomtrans)
                TransitTemp=TravellerTripOriginTransitMAZ(i,j)  
...
!                    TravellerTripParkingTaz(i,jj)=TravellerTripParkingTaz(i,jj-1)
!                enddo
...
!                    randomtrans=ceiling(rand(0)*NumTransitZoneCa(TravellerTripOriginMAZ(i,j)))
!                    otran_maz=TransitZone(TravellerTripOriginMAZ(i,j),)
!                    TravellerTripDestinationMAZ(i,j)=otran_maz
...
!                    TravellerTripParkingTaz(i,j)=TravellerTripParkingTaz(i,j)
...
!                    TravellerTripOriginMAZ(i,j+1)=0
!                    TravellerTripOriginTAZ(i,j+1)=0
!                    !TravellerTripOriginMappedTaz(i,j+1)=TAZMap(TravellerTripOriginTAZ(i,j+1))
!                    TravellerTripStartTimeInterval(i,j+1)=TravellerTripStartTimeInterval(i,j) !Need to be modified in the future (now we have used 0 minute as travel time). This should be added as Car travel time from Origin zone to Transit Origin Zone 
...
!                    TravellerTripParkingTaz(i,j+1)=TravellerTripParkingTaz(i,j)
...
!                        TripStartTime(i,j+1)=TripStartTime(i,j)+trip_travel_time
!                    endif
!                    !This is to ensure the travel time will not 
!                    if(TripStartTime(i,j+1).gt.1440) pause
!                    
...
!                    else 
!                        TravellerTripStartTimeInterval(i,j+1)=int(ceiling((TripStartTime(i,j+1)-120)/30.0)+1)
!                    endif
...
!   NumCar=0
   do i=1,index
        ...
        do j=1,TravellerNumTrips(i)-1
            ...
            if(TripDriverPassengerFlag(i,j).gt.0) then !car trip
...
!                        if(TravellerTripDestinationParkingTaz(i,j).ne.TravellerTripDestinationMAZ(i,j)) then
!                            TravellerTripDestinationMAZ(i,j)=TravellerTripDestinationParkingTaz(i,j)
!                            TravellerTripDestinationTAZ(i,j)=TravellerTripDestinationParkingTaz(i,j)
!                            ValidTraveller(i)=1
!                        endif
...
!            if (LatestCarTrip.gt.0) then                                        
!                if (TravellerTripDestinationMAZ(i,LatestCarTrip).ne.TravellerTripOriginMAZ(i,j)). then
!                    !write(3,*) 'The car trips order is not valid for car',Traveller_ID(i),'and trip',j
!                endif                                        
!            endif
!            LatestCarTrip=j
!        endif  
...
!------------------------------------------
!Assign PNR Traveller OTAP
write(*,*) "Assigning Park&Ride Traveler Transit Maz"
       ...
       do k=1,NumTransitPassengerInTimeInterval(tt)
            ...
            do j=1,TravellerNumTrips(i)   !go over all the trips   
                if (TravellerTripMode(i,j).ge.9.and.TravellerTripMode(i,j).le.12) then
                    ...
                    write(18,10) Traveller_ID(i),i,j,TripStartTime(i,j),TravellerTripOriginMAZ(i,j),TravellerTripDestinationMAZ(i,j),TravellerTripParkRideFlag(i,j)
                    traveltimetemp=10.0
...
do i=1,index
    ctime=0
    nnntrip=0
    maxnumtripsametimeinterval=0
    do j=1,TravellerNumTrips(i)
        if(ctime.eq.TravellerTripStartTimeInterval(i,j)) then 
            nnntrip=nnntrip+1
        else
            nnntrip=1
            ctime=TravellerTripStartTimeInterval(i,j)
        endif
        
        if(nnntrip.gt.maxnumtripsametimeinterval) maxnumtripsametimeinterval=nnntrip
    enddo
    NumTravelerMaxNumTrip(maxnumtripsametimeinterval)=NumTravelerMaxNumTrip(maxnumtripsametimeinterval)+1
enddo
...
!            StartTime=(nint(TravellerStartTime(i)*temp1))/temp1
            ...
            do j=1,TravellerNumTrips(i)-1  !go over all the trips except the last trip
                ...
                if (TravellerTripStartTimeInterval(i,j+1).eq.TravellerTripStartTimeInterval(i,j)) then !Zero activity time between two consequent trips with the same start time interval
                    ...
                    ActivityTimeTemp=temp3*(TravellerTripStartTimeInterval(i,j+1)-TravellerTripStartTimeInterval(i,j))-temp3*temp1
                    temp1=1.0
                ...
                write(8,13) TravellerTripDestinationTAZ(i,j),ActivityTime,TravellerTripMode(i,j),TravellerTripOriginPurpose(i,j),TravellerTripDestinationPurpose(i,j),TravellerTripOriginMAZ(i,j),TravellerTripOriginTAZ(i,j),TravellerTripDestinationMAZ(i,j),TravellerTripDestinationTAZ(i,j),TripDriverPassengerFlag(i,j),TravellerTripJointFlag(i,j),TravellerTripParkRideFlag(i,j),TravellerTripStartTimeInterval(i,j),TripStartTime(i,j),ABMTripIndex(i,j),TravellerTripDriver(i,j),TravellerTripDestinationParkingTaz(i,j)
                if(TravellerTripOriginTAZ(i,j).eq.1.and.TravellerTripDestinationMAZ(i,j).eq.4) write(*,*) i, Traveller_ID(i),j
...
11  format(3i7,f8.2,6i6,2f8.4,i5,f8.4,i5)
...
18 format('person_id	person_index	trip_index  	tripstarttim 	origmaz    	destmaz	trip_park_ride_flag	otran_maz	dtran_maz	a_time	e_time	tr_time')
